...
# Barebone StringScanner implementation (since MRuby lacks this class)
class StringScanner
  def initialize(str)
    @match = nil
    @str = str
  ...
  def scan(pattern)
    ...
    if @match
      @str = @str[(@match[0].length)...@str.length]
      @match[0]
    else
      nil
    end
...
class ERB
  ...
  class Compiler # :nodoc:
    ...
    def content_dump(s) # :nodoc:
      # Mruby has no String::count
      #n = s.count("\n")
      n = s.each_char.select { |c| c == "\n" }.length
    ...
    # Compiles an ERB template into Ruby code.  Returns the code as a string.
    def compile(s)
      s = s.dup
      out = Buffer.new(self)
